...
- [X] T102 Перед merge в `main` устранить регрессии существующих проверок: нормализовать `site` ссылки «Томато» и «Шале у реки» до корня сайта и сохранить тестовую подмену фабрики маркерных изображений для активного состояния в `src/components/map/markerImages.ts`. Проверка: `pnpm test:unit`, `pnpm build`.
