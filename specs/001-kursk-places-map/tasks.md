...
- [X] T136 Добавить web-safe ассеты портала из `/Users/cherkalexander/Downloads/Telegram Desktop/photo_2026-06-17_08-57-38.jpg`: пропорциональную обложку и историческую квадратную миниатюру портала.
...
- [X] T157 По ручному уточнению заменить фото места «Рис и Тесто» на карте «Дозаправка»: скачать переданный Instagram CDN JPEG, подготовить историческую квадратную миниатюру 480×480 и обновить прежний контракт `image`/`thumbnail` в `public/data/dozapravka-objects.json`. Проверка: `file` для новых ассетов, размер миниатюры через `sips`, `jq empty public/data/dozapravka-objects.json`, `pnpm typecheck`, `pnpm build`; новое тестовое покрытие не добавлялось.
...
- [X] T215 По утверждённому пользователем тексту обновить описание, обложку и официальную VK-ссылку места «Кухмистерская Atilan» (`id: 1619`) на основной карте: обработать `ChatGPT Image Aug 11, 2026, 08_49_36 AM.png` в WebP-обложку и квадратную миниатюру 480×480, обновить `image`/`thumbnail` и добавить `https://vk.ru/kuxmesterck_ussr` в `public/data/main-map.json`. Проверка: `jq empty public/data/main-map.json`, `file` и размеры обоих ассетов, visual preview миниатюры; новое тестовое покрытие не добавлялось.
...
- [X] T213 Обновить обложку и миниатюру места «Усадьба Афанасия Фета» (`id: 91`) только в `public/data/main-map.json` из локального файла `/Users/cherkalexander/Downloads/ChatGPT Image Jul 10, 2026, 09_50_07 PM.png`; создать hash-именованные WebP-ассеты, удалить старые неиспользуемые ассеты и поднять `package.json.version` до `0.2.3` как patch. Новое автоматизированное тестовое покрытие не добавлялось. Проверка: `jq empty public/data/main-map.json public/data/elena-koltysheva-objects.json`, dry-run helper-а перед записью, проверка WebP-размеров через `file`.

---

## Phase 54: Кнопка вписывания видимых мест

**Цель**: дать пользователю явное действие для возврата ко всему текущему набору маркеров после pan/zoom.

- [X] T214 Добавить над zoom-контролами плавающую кнопку «Показать все места на карте» на базе `IconButton`: переиспользовать общий `fitBounds` с адаптивными padding, `maxZoom: 14` и reduced-motion, вписывать текущий набор после поиска и категории без сброса состояния, отключать действие при нуле мест, синхронизировать позицию с safe areas, уведомлениями и detail-панелью. Добавить typed-событие `map_fit_places_clicked`, синхронизировать `spec.md`, `plan.md`, `tasks.md` и `contracts/analytics-events.md`, поднять `package.json.version` до `0.3.0` как minor-функцию. Новое автоматизированное тестовое покрытие не добавлялось по решению пользователя. Проверка: `pnpm typecheck`, `pnpm build`, `pnpm test:unit` (`69` passed), существующие e2e первого экрана, взаимодействия с картой и поиска с `--workers=1` (`11` passed, `1` skipped); Playwright visual/interaction QA на `1280×720` и `390×844` подтвердила выравнивание над zoom-группой, адаптацию к desktop detail-панели, disabled-состояние при нуле результатов и изменение масштаба с `14.2` до `7.7` после ручного fit.
...
- [X] T215 Подключить обязательный CARTO Basemaps API key без сохранения значения в git: добавить focused `src/config/cartoBasemap.ts`, передать helper в MapLibre `transformRequest`, документировать `VITE_CARTO_BASEMAP_API_KEY` в `.env.example`, передать production-значение из GitHub repository secret `CARTO_BASEMAP_API_KEY`, синхронизировать `spec.md`/`plan.md`/`tasks.md` и поднять `package.json.version` до `0.3.2` как patch. Новое автоматизированное тестовое покрытие не добавлялось по решению пользователя. Проверка пройдена современным bundled Node из-за системного Node 16: прямой `node ./node_modules/typescript/bin/tsc -b --pretty false`, прямой `node ./node_modules/vite/bin/vite.js build`, копирование `dist/index.html` в `dist/404.html`, `git diff --check` и поиск префикса выданного ключа в отслеживаемых и новых исходниках без совпадений.
- [ ] T216 После сохранения пользователем GitHub repository secret `CARTO_BASEMAP_API_KEY` запустить production deployment и выполнить network-smoke `kudakursk.ru`, подтвердив tile-запросы с параметром `key` без watermark.
- [X] T217 Оптимизировать медиа, initial load и доступность: перевести 109 мест на обязательные `image`/`mapThumbnail`; для исторических записей перенести прежние `thumbnail` в `/place-map-thumbnails/` байт-в-байт, сохранив визуальный кадр маркера, а WebP 320×320 quality 82 генерировать только для новых мест. Перенести логотипы под-карт в `brand`, удалить только устаревшие ссылки-миниатюры; разделить MapLibre и карточку через lazy chunks, локализовать Playfair Display, повысить контраст и исправить accessible name бренда. Проверка: сравнение production и локальной main-карты подтвердило отсутствие изменения пикселей всех 52 маркеров. Обновить существующие tests/contracts/skills и поднять версию до `0.3.3`. Production after-Lighthouse выполняется после отдельной публикации.
- [X] T218 Обновить карту Елены Колтышевой: заменить описание портала на основной карте, обновить обложку и добавить обзор «Ландизайна» только в под-карте, добавить места «VR аттракционы», «Студия мозаики», «Экскурсия на ретро-трамвае» и «Клуб спортивного покера» с локальными WebP-ассетами, координатами, официальными ссылками и Instagram-обзорами; поднять версию до `0.3.4` как patch. Проверка пройдена: `jq empty`, 19 мест и совпадающие ID `9014–9017`, формат и размеры WebP, существующий `mapDataIntegrity` unit-тест (3/3), typecheck и production build прямыми локальными командами через Node 22 из-за системного Node 16. Playwright visual QA на desktop 1280×720 и mobile 390×844 подтвердила новое описание портала, загрузку изображений и ссылок всех пяти обновлённых карточек, наличие маршрутов, отсутствие horizontal overflow и ошибок консоли; новое автоматизированное покрытие не добавлялось.

- [X] 2026-08-18 Для бахчи «КурскАрбуз» (ID 1430) на главной карте заменены обложка и миниатюра на предоставленное фото: созданы WebP 1200×1600 и 480×480, прежние неиспользуемые ассеты удалены; описание обновлено на краткий креативный текст об эко-арбузах и «Сено парке». Проверка: JSON успешно распарсен, размеры и формат WebP проверены, визуальная проверка обложки выполнена; новое автоматизированное покрытие не добавлялось.
...
- [X] 2026-07-27 Добавлены NoCulture, «Рюмочная „Однушка“», «Настойкин» и Bebop только на главную карту с локальными WebP-обложками и миниатюрами, адресами, координатами, краткими описаниями и подтверждёнными ссылками на соцсети; по ручному уточнению заменена ссылка ВКонтакте NoCulture, добавлен Instagram «Настойкина» и удалён Telegram Bebop; карта «Дозаправка» не изменялась. Проверка: `jq empty public/data/main-map.json`, проверка WebP-файлов и миниатюр, `pnpm typecheck`, `pnpm build`; новое автоматизированное покрытие не добавлялось.
