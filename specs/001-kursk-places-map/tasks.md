...
- [X] 2026-08-24 Создана под-карта «Фотоистория Курска»: загрузчик VK совместим со старым Node и считывает ключ только из окружения/локального `.env`; выгружены 100 уникальных постов и 218 исходных фотографий в игнорируемый кэш. Добавлены 13 подтверждённых точек, WebP-ассеты, портал на основной карте и CSV-отчёт из 100 строк. Защищено обновление данных MapLibre при размонтировании карты; переход в под-карту и обратно не даёт browser errors. Выполнены typecheck, production-сборка и visual QA desktop/mobile; новые автоматические тесты не добавлялись.
- [X] 2026-08-24 Повторно обработаны исключённые посты и выгружена страница `offset=100`: исправлен загрузчик VK для удалённых слотов и закреплённых записей, добавлены 37 новых исторических точки с WebP-ассетами. Под-карта содержит 50 мест; новый `reports/fotohistory-kursk-200.csv` имеет 200 строк, включая 50 решений `included`. Для исторических улиц и перекрёстков без номера здания указана ориентировочная геопривязка. Проверены JSON, адреса, координаты, desktop/mobile deep link и карточка; новые автоматические тесты не добавлялись.
- [X] 2026-08-24 Для 50 опубликованных фотографий «Фотоистории Курска» добавлены поля `balloonContent.images[].caption` с дословными текстами исходных постов VK; эти же тексты заменили редакционные пересказы в описаниях карточек. Проверка: все 50 точек сопоставлены с локальными снимками выгрузки, пустых подписей нет.
- [X] 2026-08-24 Для 50 точек «Фотоистории Курска» добавлены редакционные категории: ровно одна эпоха и одна-две темы. Под-карта получила доступный переключатель «Эпохи / Сюжеты», URL-параметр `categoryGroup`, метку эпохи в карточке и typed-событие выбора фильтра. Обновлены Storybook-состояния, аналитический контракт и версия до `0.5.0`; новое автоматическое тестовое покрытие не добавлялось.
