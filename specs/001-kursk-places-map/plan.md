...
- Каждая карта несёт собственную идентичность в каталоге (`mapCatalog`): поля `logo`, `title`, `description`. При переходе на под-карту бренд-блок (`MapLogo`) показывает логотип, название и описание этой карты; на desktop и mobile он сохраняет высоту поля поиска (`48px`), имеет компактную базовую ширину и растёт до максимума только когда этого требует контент. Описание показывается в одну строку и обрезается при нехватке ширины. Под-карты переиспользуют существующие ассеты (фото места-входа как логотип «Дозаправка», отдельный webp-логотип «Запишу, зарисую»).
