...
- Каждая карта несёт собственную идентичность в каталоге (`mapCatalog`): поля `logo`, `title`, `description`. При переходе на под-карту бренд-блок (`MapLogo`) показывает логотип, название и описание этой карты. Под-карты переиспользуют существующие ассеты (фото места-входа как логотип «Дозаправка», отдельный webp-логотип «Запишу, зарисую»).
