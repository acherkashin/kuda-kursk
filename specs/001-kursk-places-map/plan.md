...
- `AboutProjectDialog` показывает логотип, редакционное описание проекта, placeholder-ссылки на Telegram и форму обратной связи, а также секцию управления analytics consent.
...
- Контент проекта вынесен в отдельный доменный модуль, чтобы placeholder-ссылки можно было заменить без изменения UI-компонентов.
...
Подход переиспользует уже созданный источник `projectInfo.feedbackUrl` и не добавляет новую форму или данные места:

- В нижнюю часть `PlaceDetailsPanel` добавляется маленькое muted-действие с иконкой сообщения о проблеме.
- Ссылка открывает ту же форму обратной связи, что и диалог «О проекте», без дополнительных query-параметров и без отправки новых analytics events.
