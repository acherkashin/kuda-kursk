...
- Обязательные поля карточки: `name`, `description`, `address`, `image` и `mapThumbnail`.
...
**Поля**
- `image`: обязательный путь к пропорциональному изображению карточки.
- `mapThumbnail`: обязательный путь к отдельному изображению маркера; для исторических мест сохраняет прежнее кадрирование, для новых создаётся как центрированная квадратная миниатюра.
...
- `description`: контекст аудитории / подзаголовок бренд-блока (реализовано в `mapCatalog`).
- `logo`: путь к логотипу карты, показываемому в бренд-блоке (реализовано в `mapCatalog`; у под-карт — фото места-входа или собственный webp).
- `placeIds`: публичные места основной карты.
- `linkOnlyPlaceIds`: места, видимые только внутри этой карты сообщества.
- `theme`: optional визуальные настройки акцента.
