...
export function PlacePhotoGallery({ photos, placeId, title, onClose }: PlacePhotoGalleryProps) {
  ...
  return (
    <section
      className="relative h-[min(65dvh,520px)] overflow-hidden bg-[var(--color-text)] outline-none [touch-action:pan-y] focus-visible:outline-2 focus-visible:outline-offset-[-3px] focus-visible:outline-[var(--color-accent)]"
      role="region"
    ...
    >
      <motion.img
        key={activePhoto.src}
        className={imageStatus === "error" ? "block h-full w-full object-contain opacity-0" : "block h-full w-full object-contain"}
      ...
      {imageStatus === "error" ? (
        <div
          className="absolute inset-0 z-1 grid place-content-center gap-2 px-6 text-center text-[13px] text-white/80"
          role="status"
        >
      ...
      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_top,rgba(20,14,8,0.82)_0%,rgba(20,14,8,0.42)_28%,rgba(20,14,8,0.08)_50%,transparent_66%)]" />

      {hasMultiplePhotos ? (
        <>
          <span
            className="absolute top-4 left-4 rounded-full border border-white/20 bg-black/45 px-2.5 py-1 text-[12px] font-semibold text-white tabular-nums backdrop-blur-sm max-[700px]:top-[max(16px,env(safe-area-inset-top))]"
            aria-live="polite"
            aria-atomic="true"
          >
      ...
      <div className="pointer-events-none absolute right-0 bottom-0 left-0 px-5 pt-14 pb-5">
